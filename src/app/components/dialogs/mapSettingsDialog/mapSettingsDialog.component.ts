...
export class MapSettingsDialogComponent implements OnInit {
  ...
  public applyChanges(): void {
    console.debug('applyChanges', this.generalSelectionValue);

    if (this.generalSelectionValue[0] !== ColourPaletteType.CUSTOM) {
      this.selectedPalette = PALETTES.find((palette: ColourPalette) => palette.name === this.generalSelectionValue[0]);
    } else {
