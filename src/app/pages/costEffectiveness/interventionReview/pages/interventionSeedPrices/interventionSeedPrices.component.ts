...
import { CropProductionInformation } from 'src/app/apiAndObjects/objects/interventionCropProductionInformation';
...
@Component({
  selector: 'app-intervention-seed-prices',
  templateUrl: './interventionSeedPrices.component.html',
  styleUrls: ['./interventionSeedPrices.component.scss'],
})
export class InterventionSeedPricesComponent implements OnInit {
  public dirtyIndexes = [];
  ...
  private initFormWatcher(): void {
    ...
    if (null != activeInterventionId) {
      void this.interventionDataService
        .getInterventionSeedPrices(activeInterventionId)
        .then((data: InterventionSeedPrices) => {
          this.dataSource = new MatTableDataSource(data.seedPrices);
          const cropProductionGroupArr = data.seedPrices.map((item) => {
  ...
  get cropProductionArray(): UntypedFormArray {
    return this.form.get('items')['controls'] as UntypedFormArray;
  }

  private createCropProductionGroup(item: CropProductionInformation): UntypedFormGroup {
