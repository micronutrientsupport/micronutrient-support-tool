...
@Component({
  ...
  styleUrls: ['./interventionCropProductionProjections.component.scss'],
})
export class InterventionCropProductionProjectionsComponent implements OnInit {
  public dirtyIndexes = [];
    ...
    if (null != activeInterventionId) {
      void this.interventionDataService
        ...
        .then((data: InterventionCropProductionInformation) => {
          const filtered = data.cropProductionInformation.filter(
            (row) => row.rowName === 'area_crop_production_human_consumption',
          );

          this.dataSource = new MatTableDataSource(filtered);
          const cropProductionGroupArr = filtered.map((item) => {
