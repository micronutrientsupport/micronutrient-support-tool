...
export class InterventionSeedPrices extends BaseObject implements Named {
  ...
  public readonly seedPrices: Array<SeedPriceInformation>;

  protected constructor(sourceObject?: Record<string, unknown>) {
    super(sourceObject);

    this.interventionId = this._getNumber(InterventionSeedPrices.KEYS.INTERVENTION_ID);
    this.seedPrices = this._getArray(InterventionSeedPrices.KEYS.SEED_PRICES);
  }
}

export interface SeedPriceInformation {
  year0: number;
  year1: number;
  year2: number;
  year3: number;
